...
export default function MiniCard({ c, showFlag = true }: { c: MiniCardData; showFlag?: boolean }) {
  return (
    <Link
      href={`/player/${c.id}`}
    ...
    >
      ...
      <p className="eyebrow mt-0.5 flex items-center justify-center gap-1.5">
        {showFlag && <img src={flagUrl(c.abbr)} alt="" width={14} height={14} className="rounded-[2px]" loading="lazy" />}
        {c.pos}
      </p>
    </Link>
  );
